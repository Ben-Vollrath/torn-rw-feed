...
class Fetcher : public oatpp::async::Coroutine<Fetcher> {
  ...
  std::chrono::microseconds now =
      std::chrono::microseconds(oatpp::base::Environment::getMicroTickCount());
 ...
 public:
  ...
  Action act() override {
    ...
    return m_tornApiService.getFactionWarAndMembers().callbackTo(
        &Fetcher::onFactionWarAndMembersResponse);
  }

  Action onFactionWarAndMembersResponse(
    ...
    if (factionWarResponse->basic->id !=
        m_factionId)  // Key is outdated (member moved to another faction)
    {
      auto faction =
          m_factionService.getByIdNullable(factionWarResponse->basic->id);
      if (!faction) {
        m_factionService.create(
            FactionDto::fromFactionResponse(factionWarResponse->basic));
      }
      m_userService.updateFaction(m_tornApiService.getLastKey()->torn_key,
                                  factionWarResponse->basic->id);
  ...
  Action onEnemiesUpdate(
    ...
    if (room->needStats()) {
      auto stats = m_memberStatsService.getAllForWar(m_warId.value(),
                                                     m_enemyFactionId.value());

      if (stats->empty()) {
        return m_ffScouterApiService.getScout(memberInfo)
            .callbackTo(&Fetcher::onScouts);
      }
  ...
  Action onScouts(const FFScouterResponseDto& scouts) {
    auto stats =
        m_memberStatsService.createMany(MemberStatsDto::fromFFScouterResponse(
            m_warId.value(), m_enemyFactionId.value(), scouts));
  ...
  Action handleError(Error* e) override {
    ...
    const auto waitTime = std::max(clock::zero(), next - now);

    try {
      OATPP_LOGE(TAG, "Fetcher error: %s",
                 e && e->what() ? e->what() : "unknown");
      const int code = std::atoi(e && e->what() ? e->what() : "0");
      if (code == 429) {
        auto room = m_room.lock();
        if (room) {
          room->sendError(ErrorMessage::KeyLimit,
                          m_tornApiService.getLastKey()->userId);
        }
      }
 ...
 private:
  void setNextExecTimer(std::chrono::microseconds& timer) {
    timer = std::chrono::microseconds(
        std::max((timer + m_interval).count(),
