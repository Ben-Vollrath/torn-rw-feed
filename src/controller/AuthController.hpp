...
class AuthController : public oatpp::web::server::api::ApiController {
 ...
 public:
  ...
  ENDPOINT_INFO(authorizeUser) {
    ...
    info->addResponse<Object<ApiKeyIssueResponseDto>>(Status::CODE_200,
                                                      "application/json");
    info->addTag("Auth");
  }
  ...
  ENDPOINT_ASYNC("GET", "/auth", authorizeUser) {
    ...
    oatpp::async::Action act() override {
      ...
      return controller->m_tornApiService.getFactionBasic(m_tornKey).callbackTo(
          &authorizeUser::upsertFaction);
    }

    oatpp::async::Action upsertFaction(
        const oatpp::Object<TornFactionResponseDto>& factionBasics) {
      // Update faction information
      oatpp::Object<FactionDto> faction =
          FactionDto::fromFactionResponse(factionBasics->basic);
      ...
      return controller->m_tornApiService.getKeyInfo(m_tornKey).callbackTo(
          &authorizeUser::createUserAndRespond);
    }

    oatpp::async::Action createUserAndRespond(
        const oatpp::Object<TornKeyResponseDto>& keyInfo) {
      if (keyInfo->info->access->type == TornKeyAccessType::CUSTOM) {
        return _return(controller->createResponse(
            Status::CODE_401, "Custom keys are not allowed"));
      }
      ...
      return _return(
          controller->createDtoResponse(Status::CODE_200, keyResponse));
    }
