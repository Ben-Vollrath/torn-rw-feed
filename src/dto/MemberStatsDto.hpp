...
class MemberStatsDto : public oatpp::DTO {
  ...
  static oatpp::Object<MemberStatsDto> fromFFScouterItem(
      std::int64_t warId, std::int64_t factionId,
  ...
  static oatpp::Vector<oatpp::Object<MemberStatsDto>> fromFFScouterResponse(
      std::int64_t warId, std::int64_t factionId,
      const FFScouterResponseDto& rsp) {
  ...
  static oatpp::Object<MemberStatsDto> fromTornStatsSpyItem(
      std::int64_t warId, std::int64_t factionId, std::int64_t memberId,
      const oatpp::Object<SpyDto>& spy) {
    auto dto = createShared();
    dto->total = spy->total;
  ...
  static oatpp::Vector<oatpp::Object<MemberStatsDto>> fromTornStatsSpyResponse(
      std::int64_t warId, std::int64_t factionId,
      const oatpp::Object<TornStatsSpyResponseDto>& spy) {
    auto out = oatpp::Vector<oatpp::Object<MemberStatsDto>>::createShared();
    for (const auto& pair : *spy->faction->members) {
      ...
      out->emplace_back(fromTornStatsSpyItem(
          warId, factionId, std::stoll(pair.first), pair.second->spy));
    }
    return out;
  }
};
