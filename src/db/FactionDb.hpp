...
public:
		...
		: DbClient(executor)
	{
		...
		m.addFile(1, SQL_FILE_PATH"factions/init.sql");
		m.migrate();
	}
	...
	QUERY(upsertById,
	      ...
	      "  leader_torn_id = COALESCE(EXCLUDED.leader_torn_id, factions.leader_torn_id), "
	      "  co_leader_torn_id = COALESCE(EXCLUDED.co_leader_torn_id, factions.co_leader_torn_id) "
	      "RETURNING *;",
	      PARAM(oatpp::Object<FactionDto>, f))
