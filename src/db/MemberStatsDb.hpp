...
class MemberStatsDb : public oatpp::orm::DbClient {
 ...
 public:
  ...
  QUERY(create,
        "INSERT INTO member_stats (war_id, faction_id, member_id, type, str, "
        "def, spd, dex, total) "
        "VALUES (:memberStats.war_id, :memberStats.faction_id, "
        ":memberStats.member_id, :memberStats.type, :memberStats.str, "
        ":memberStats.def, :memberStats.spd, :memberStats.dex, "
        ":memberStats.total) "
        "RETURNING *;",
  ...
  QUERY(deleteAll, "DELETE FROM member_stats;")
};
