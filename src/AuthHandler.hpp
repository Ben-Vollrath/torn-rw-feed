...
class AuthHandler
 ...
 public:
  AuthHandler() : BearerAuthorizationHandler("Authorization") {}
  ...
  std::shared_ptr<AuthorizationObject> authorize(
    ...
    OATPP_ASSERT_HTTP(parts.size() == 3 && parts[0] == "ak", Status::CODE_401,
                      "401");
    ...
    OATPP_ASSERT_HTTP(!(key->revoked && *key->revoked), Status::CODE_401,
                      "401");
    ...
    const std::string presentedHashHex = CryptoUtils::sha256Hex(
        reinterpret_cast<const CryptoPP::byte*>(secretBytes.data()),
        secretBytes.size());

    // Normalize stored hex (strip Postgres bytea "\x" prefix if present)
    std::string storedHex =
        key->secretHash ? std::string(key->secretHash->c_str()) : "";
    if (storedHex.size() >= 2 && storedHex[0] == '\\' && storedHex[1] == 'x') {
      storedHex.erase(0, 2);
    }

    // Constant-time compare
    OATPP_ASSERT_HTTP(ct_equal(storedHex, presentedHashHex), Status::CODE_401,
                      "401");
