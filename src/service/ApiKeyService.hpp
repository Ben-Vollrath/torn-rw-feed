...
class ApiKeyService : public CrudService<ApiKeyDb, ApiKeyDto> {
 public:
  ...
  IssueResult issueKey(const oatpp::Int64& userId,
                       const std::optional<int64_t>& ttlSeconds =
    ...
    std::string prefixHex =
        CryptoUtils::hexEncode(prefixBytes.data(), prefixBytes.size());
    std::string secretB64 =
        CryptoUtils::base64URLEncode(secretBytes.data(), secretBytes.size());

    // hash secret (store only hash)
    std::string secretHashHex =
        CryptoUtils::sha256Hex(secretBytes.data(), secretBytes.size());
