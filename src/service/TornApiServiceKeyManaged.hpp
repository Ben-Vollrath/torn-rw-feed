 ...
 public:
  ...
  getFactionMembers(const std::int64_t factionId) {
    ...
    return callWithKeyManagementAsync<Result>(
        &TornApiService::getFactionMembers, factionId);
  }
  ...
  getFactionWarAndMembers() {
    ...
    return callWithKeyManagementAsync<Result>(
        &TornApiService::getFactionWarAndMembers);
  }
  ...
  getEnemyWarFaction(const std::int64_t factionId) {
    ...
    return callWithKeyManagementAsync<Result>(
        &TornApiService::getEnemyWarFaction, factionId);
  }
    ...
    class Coroutine
     ...
     public:
      Coroutine(TornApiServiceKeyManaged* self, MemFn mf, Args&&... args)
          : m_self(self), m_mf(mf), m_args(std::forward<Args>(args)...) {}
      ...
      Action returnResponse(const Result& r) { return this->_return(r); }

      Action handleError(oatpp::async::Error* e) override {
        OATPP_LOGE("KeyManaged", "Fetcher error: %s",
                   e ? e->what() : "unknown");
