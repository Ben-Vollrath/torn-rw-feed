...
class TornApiService {
  ...
  class ApiActionBase : public oatpp::async::CoroutineWithResult<T, TDto> {
   ...
   public:
    ApiActionBase(TornApiService* apiService,
  ...
  class FactionApiActionBase : public ApiActionBase<T, TDto> {
   ...
   public:
    FactionApiActionBase(
        TornApiService* apiService,
        const std::shared_ptr<TornApiClient>& client,
        const std::shared_ptr<oatpp::data::mapping::ObjectMapper>& om,
        std::string key, std::int64_t factionId)
  ...
  oatpp::Object<DtoT> parseSafely(const oatpp::String& body) {
    auto rsp =
        objectMapper->readFromString<oatpp::Object<TornErrorResponseDto>>(body);
 ...
 public:
  ...
  getFactionBasic(const std::string& key);
  oatpp::async::CoroutineStarterForResult<
      const oatpp::Object<TornKeyResponseDto>&>
  getKeyInfo(const std::string& key);
  oatpp::async::CoroutineStarterForResult<
      const oatpp::Object<TornFactionMembersResponse>&>
  getFactionMembers(const std::string& key, std::int64_t factionId);
  oatpp::async::CoroutineStarterForResult<
      const oatpp::Object<TornFactionWarResponseDto>&>
  getFactionWar(const std::string& key);
  oatpp::async::CoroutineStarterForResult<const std::optional<std::int64_t>&>
  getEnemyWarFaction(const std::string& key, std::int64_t factionId);
  oatpp::async::CoroutineStarterForResult<
      const oatpp::Object<TornFactionWarAndMembersResponseDto>&>
  getFactionWarAndMembers(const std::string& key);
};
