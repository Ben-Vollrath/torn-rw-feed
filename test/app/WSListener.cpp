    ...
    if (wholeMessage) {
      {
        std::lock_guard<std::mutex> lk(m_mtx);
        auto messageDto =
            apiObjectMapper->readFromString<oatpp::Object<WarStateResponseDto>>(
                wholeMessage);
...
bool WSListener::waitForNext(oatpp::Object<WarStateResponseDto>& out,
                             std::chrono::milliseconds timeout) {
  std::unique_lock<std::mutex> lk(m_mtx);
  if (!m_cv.wait_for(lk, timeout,
                     [&] { return !m_inbox.empty() || m_closed; })) {
