...
class TestingFixtures {
 ...
 public:
  ...
  oatpp::Object<UserDto> createTestUser(
      std::optional<std::int64_t> factionId = std::nullopt,
  ...
  void deleteTestUser(std::int64_t userId) { m_userService.removeById(userId); }

  ApiKeyService::IssueResult getUserApiKey(std::int64_t userId) {
    return m_apiKeyService.issueKey(userId);
  }

  oatpp::web::protocol::http::Headers getUserAuthHeader(
      ApiKeyService::IssueResult issueResult) {
    oatpp::web::protocol::http::Headers headers;
    headers.put(oatpp::web::protocol::http::Header::AUTHORIZATION,
                oatpp::String(issueResult.fullKey));
    return headers;
  }
  ...
  oatpp::Object<TargetsDbDto> createTargets(
      oatpp::Object<TargetsDbDto> targets) {
