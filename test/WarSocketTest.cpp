...
void WarSocketTest::testSocketOk(
  ...
  mockResponseLoader->setResponsePaths(
      {factionWarAndMembersOKPath_, factionMembersOfflineOKPath_,
       ffscouterScoutOkPath_, factionWarAndMembersChangedOKPath_,
       factionMembersOneOnlineOKPath_, factionWarAndMembersChangedOKPath_,
       factionMembersOfflineOKPath_});

  auto factionWar =
      mockResponseLoader
          ->loadDtoFromFile<oatpp::Object<TornFactionWarAndMembersResponseDto>>(
              factionWarAndMembersOKPath_);
  factionWar->members[0]->status->parseLocation();
  auto factionWarChangedScore =
      mockResponseLoader
          ->loadDtoFromFile<oatpp::Object<TornFactionWarAndMembersResponseDto>>(
              factionWarAndMembersChangedOKPath_);
  factionWarChangedScore->members[0]->status->parseLocation();
  auto factionMembersOffline =
      mockResponseLoader
          ->loadDtoFromFile<oatpp::Object<TornFactionMembersResponse>>(
              factionMembersOfflineOKPath_);
  ...
  auto factionMembersOnline =
      mockResponseLoader
          ->loadDtoFromFile<oatpp::Object<TornFactionMembersResponse>>(
              factionMembersOneOnlineOKPath_);
  factionMembersOnline->members[0]->status->parseLocation();
  auto ffScouter = mockResponseLoader->loadDtoFromFile<FFScouterResponseDto>(
  ...
  auto w1 = makeTestWebSocket("/wars/socket?token=" + issueResult.fullKey,
                              clientConnectionProvider);
  ...
  OATPP_ASSERT(
      dtoFieldsEqualLogger(msg->user, factionWar->members[0], objectMapper))
  ...
  OATPP_ASSERT(dtoFieldsEqualLogger(
      msg->members[0], factionMembersOffline->members[0], objectMapper))
  OATPP_ASSERT(dtoFieldsEqualLogger(
      msg->members[1], factionMembersOffline->members[1], objectMapper))
  ...
  OATPP_ASSERT(!msg->members);
  OATPP_ASSERT(
      msg->memberStats[std::to_string(memberOneId)]["ffscouter"]->total ==
      memberOneStats);
  OATPP_ASSERT(
      !msg->memberStats[std::to_string(memberTwoId)]["ffscouter"]->total);
  ...
  OATPP_ASSERT(!msg->members);
  OATPP_ASSERT(dtoFieldsEqualLogger(
      msg->user, factionWarChangedScore->members[0], objectMapper))
  OATPP_ASSERT(dtoFieldsEqualLogger(msg->war, factionWarChangedScore->wars,
                                    objectMapper))
  ...
  OATPP_ASSERT(dtoFieldsEqualLogger(
      msg->members[0], factionMembersOnline->members[0], objectMapper))
  ...
  std::this_thread::sleep_for(std::chrono::seconds(3));  // Ensure fetcher
                                                         // closes
}

void WarSocketTest::testSocketSendCurrentState(
    std::shared_ptr<oatpp::data::mapping::ObjectMapper> objectMapper) {
  ...
  auto w2 = makeTestWebSocket("/wars/socket?token=" + issueResult2.fullKey,
                              clientConnectionProvider);

  got = w2.listener->waitForNext(msg, std::chrono::seconds(500));
  OATPP_ASSERT(got);
  OATPP_ASSERT(
      dtoFieldsEqualLogger(msg->user, factionWar->members[1], objectMapper))
  OATPP_ASSERT(dtoFieldsEqualLogger(msg->war, factionWar->wars, objectMapper))
  OATPP_ASSERT(msg->members->size() == 2);
  OATPP_ASSERT(dtoFieldsEqualLogger(
      msg->members[0], factionMembersOffline->members[1], objectMapper))
  OATPP_ASSERT(dtoFieldsEqualLogger(
      msg->members[1], factionMembersOffline->members[0], objectMapper))
...
void WarSocketTest::testSocketTooManyRequests(
    std::shared_ptr<oatpp::data::mapping::ObjectMapper> objectMapper) {
  ...
  OATPP_COMPONENT(std::shared_ptr<MockResponseLoader>, mockResponseLoader);
  mockResponseLoader->setResponsePaths(
      {errorTooManyRequests_, errorTooManyRequests_,
       factionWarAndMembersOKPath_, factionMembersOfflineOKPath_});
...
void WarSocketTest::testSocketNoWar(
    std::shared_ptr<oatpp::data::mapping::ObjectMapper> objectMapper) {
  ...
  OATPP_COMPONENT(std::shared_ptr<MockResponseLoader>, mockResponseLoader);
  mockResponseLoader->setResponsePaths(
      {factionWarAndMembersNoWarOKPath_,
       factionWarAndMembersNoWarChangedOKPath_,
       factionWarAndMembersNoWarChangedOKPath_});
  ...
  OATPP_ASSERT(!msg->members);
  OATPP_ASSERT(!msg->war);
  OATPP_ASSERT(dtoFieldsEqualLogger(msg->user, factionWarUpdated->members[0],
                                    objectMapper))
...
void WarSocketTest::testSocketTargetsUpdate(
    std::shared_ptr<oatpp::data::mapping::ObjectMapper> objectMapper) {
  ...
  mockResponseLoader->setResponsePaths(
      {factionWarAndMembersOKPath_, factionMembersOfflineOKPath_,
       ffscouterScoutOkPath_, factionWarAndMembersOKPath_});
  ...
  auto db = targetService.getAllForUser(
      factionWar->getEnemyFactionId(user->factionId).value(), user->id);
  OATPP_ASSERT(db->targets_set->size() == 1)
  OATPP_LOGD(TAG, "testSocketTargetsUpdate Completed");
}

void WarSocketTest::testSocketTargetsUpdateUserDeleted(
    std::shared_ptr<oatpp::data::mapping::ObjectMapper> objectMapper) {
...
void WarSocketTest::testSocketTargetsLoad(
    std::shared_ptr<oatpp::data::mapping::ObjectMapper> objectMapper) {
  ...
  auto user = testingFixtures->createTestUser(1);
  auto issueResult = testingFixtures->getUserApiKey(user->id);
  auto targetDbDto = TargetsDbDto::createFromFactionAndUser(
      factionWar->getEnemyFactionId(user->factionId).value(), user->id);
...
void WarSocketTest::testSocketUserSwitchedFaction(
    std::shared_ptr<oatpp::data::mapping::ObjectMapper> objectMapper) {
  OATPP_LOGD(TAG, "TestSocketOk Started");
  OATPP_COMPONENT(std::shared_ptr<TestingFixtures>, testingFixtures);
  testingFixtures->reset();
...
void WarSocketTest::testPostSpyWithRoom(
    const std::shared_ptr<ApiTestClient> client,
    std::shared_ptr<oatpp::data::mapping::ObjectMapper> objectMapper) {
  ...
  auto tornStatsSpy =
      mockResponseLoader
          ->loadDtoFromFile<oatpp::Object<TornStatsSpyResponseDto>>(
              tornStatsSpyOkPath_);
  auto memberOneSpy =
      tornStatsSpy->faction->members[std::to_string(memberOneId)]->spy;
  ...
  OATPP_ASSERT(got);
  OATPP_ASSERT(
      msg->memberStats[std::to_string(memberOneId)]["tornstatsspies"]->total ==
      memberOneSpy->total);
...
void WarSocketTest::testPostSpyWithoutRoom(
    const std::shared_ptr<ApiTestClient> client,
    std::shared_ptr<oatpp::data::mapping::ObjectMapper> objectMapper) {
  ...
  auto factionWar =
      mockResponseLoader
          ->loadDtoFromFile<oatpp::Object<TornFactionWarResponseDto>>(
              factionWarOKPath_);
  auto factionMembersOffline =
      mockResponseLoader
          ->loadDtoFromFile<oatpp::Object<TornFactionMembersResponse>>(
  ...
  auto stats = statsService.getAllForWar(
      factionWar->getWarId().value(), factionWar->getEnemyFactionId(1).value());
...
void WarSocketTest::onRun() {
  ...
  runner.run(
      [this, &runner] {
        ...
        auto client =
            ApiTestClient::createShared(requestExecutor, objectMapper);
  ...
  OATPP_COMPONENT(std::shared_ptr<oatpp::postgresql::ConnectionPool>,
                  connectionPool);
  connectionPool->stop();
