...
void AuthControllerTest::testAuthOk(
  ...
  auto factionInfo =
      mockResponseLoader
          ->loadDtoFromFile<oatpp::Object<TornFactionResponseDto>>(
              factionBasicOKPath_);
  auto keyInfo =
      mockResponseLoader->loadDtoFromFile<oatpp::Object<TornKeyResponseDto>>(
          keyInfoOKPath_);
  ...
  auto message = response->readBodyToDto<oatpp::Object<ApiKeyIssueResponseDto>>(
      objectMapper.get());
  ...
  constexpr auto seconds_in_month =
  ...
  OATPP_ASSERT(user && user->factionId == factionInfo->basic->id &&
               user->tornKey == "ok")
}

void AuthControllerTest::testAuthError(
    const std::shared_ptr<ApiTestClient> client,
    std::shared_ptr<oatpp::data::mapping::ObjectMapper> objectMapper) {
  OATPP_COMPONENT(std::shared_ptr<TestingFixtures>, testingFixtures);
  testingFixtures->reset();
  ...
  mockResponseLoader->setResponsePaths(
      {errorInactiveKey_, factionBasicOKPath_, keyInfoCustomKeyPath_});
  // Inactive key
